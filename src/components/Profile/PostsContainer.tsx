...
import ProfileCardItem from "../Profile/ProfilePostCard";
...
const PostsContainer = () => {
  ...
  return (
    <IonContent className="profilePostsContent">
      ...
      <ProfileInfo />
      <IonListHeader>
        <IonTitle className="profileTitle">
          {posts.length ? "My Posts" : "No posts yet"}
        </IonTitle>
      </IonListHeader>
      <IonList className="postList">
        {posts.map((post: any) => {
          return <ProfileCardItem key={post.id} post={post} userPost={true} />;
        })}
      </IonList>
    </IonContent>
  );
